...
1. The API deterministically records either a new observation or the result of the currently proposed test.
...
6. The service removes unknown citations, checks the proposed test against completed fingerprints, and validates risk metadata.
7. The validated assessment and an immutable event are persisted before the API responds.
...
- Proposed and completed tests, including natural-language results
...
The first implementation uses SQLite FTS5 because it is small, transparent, and works offline without a second model. Procedure cards are Markdown plus YAML metadata. Search returns a bounded snippet and the card's stable ID.
...
Anything marked destructive must include prerequisites, a backup/rollback statement, and `requires_confirmation=true`. The API refuses an invalid proposal. A future command runner would be a separate privileged component and is explicitly outside this architecture.
...
- If the model is unavailable, the recorded observation/result remains saved and the UI reports that reasoning is pending.
